...
├── walkable.ex                # AST/Type traversal protocol (Walker module)
...
│   └── builtins.ex            # Built-in function signatures and guards
│
...
Protocol implementations are generated by `use Deft.AST.Node` in each AST module. The `Deft.Walker` module (in `walkable.ex`) provides `postwalk/2` and `prewalk/2` for transformation.
